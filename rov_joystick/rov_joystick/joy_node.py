...
class ROVJoyNode(Node):
    def __init__(self):
        ...
        # --- BUTTON MAPPING (Logitech / Xbox Standard) ---
        self.AXIS_SURGE = 1  # Left Stick Y
        self.AXIS_YAW   = 0  # Left Stick X
        self.AXIS_HEAVE = 4  # Right Stick Y

        self.BTN_ARM    = 0  # 'A' (Cross) -> Arm/Disarm
        self.BTN_STOP   = 1  # 'B' (Circle) -> EMERGENCY KILL
        self.BTN_LOCK   = 3  # 'Y' (Triangle)-> Depth Lock
    ...
    def publish_cmd(self):
        ...
        if curr_stop == 1 and self.last_states['stop'] == 0:
            ...
            # If Emergency active, force disarm locally
        ...
        if curr_arm == 1 and self.last_states['arm'] == 0:
            # Can only arm if Emergency is OFF
        ...
        # --- 4. MOVEMENT ---
        msg = Twist()
        # Only read sticks if Armed AND Safe
        if self.armed and not self.emergency_state:
